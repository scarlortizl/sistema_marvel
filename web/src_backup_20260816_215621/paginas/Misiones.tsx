...
function nivelNumero(nivel: Mision['nivel_peligro']) {
  if (nivel === 'BAJO') return 34;
  if (nivel === 'MEDIO') return 67;
  return 100;
}
...
export function Misiones() {
  ...
  return (
    <section className="screen mission-editorial mission-editorial--v8 mission-editorial--v9" ref={rootRef}>
      ...
      {cargando ? (
        ...
        <div className="mission-grid">
          {misiones.map((mision, indice) => (
            <article
              className={`mission-card mission-card--${mision.nivel_peligro.toLowerCase()}`}
              key={mision.id}
              style={
                {
                  '--delay': `${indice * 70}ms`,
                  '--risk-value': nivelNumero(mision.nivel_peligro),
                } as CSSProperties
              }
              onMouseMove={moverGlow}
            >
              <div className="mission-card__glow" aria-hidden="true" />
              ...
              <div className="mission-card__media">
                ...
                <div className="mission-card__noise" aria-hidden="true" />
                <div className="mission-card__overlay" />
                <div className="mission-card__topline">
                  <span className="mission-card__id">ID : MSP {String(mision.id).padStart(2, '0')}</span>
                  <span className={`etiqueta etiqueta--${mision.estado.toLowerCase()}`}>{mision.estado}</span>
                </div>
              </div>

              <div className="mission-card__content">
                <span className="mission-card__eyebrow">TACTICAL OPERATION</span>
                <h2>{mision.titulo}</h2>
                <p className="mission-card__unit-name">
                  {mision.superheroe ? mision.superheroe.nombre : 'Unidad no asignada'}
                </p>
                <p className="mission-card__description">{mision.descripcion}</p>

                <div className="mission-card__specs">
                  <span><strong>LOC</strong><em>{mision.ubicacion}</em></span>
                  <span><strong>DATE</strong><em>{new Date(mision.fecha).toLocaleDateString('es-EC')}</em></span>
                </div>

                <div className="mission-card__bottom">
                  <div className="mission-card__risk-block">
                    <div className="mission-card__risk-head">
                      <span>PRM_LVL</span>
                      <strong>{nivelNumero(mision.nivel_peligro)}%</strong>
                    </div>
                    <div className="mission-card__risk-track">
                      <div
                        className={`mission-card__risk-fill mission-card__risk-fill--${mision.nivel_peligro.toLowerCase()}`}
                        style={{ width: nivelAncho(mision.nivel_peligro) }}
                      />
                    </div>
                  </div>

                  <div className={`mission-card__ring mission-card__ring--${mision.nivel_peligro.toLowerCase()}`}>
                    <div className="mission-card__ring-core">
                      <strong>{nivelNumero(mision.nivel_peligro)}</strong>
                    </div>
                  </div>
                </div>
                ...
                {esAdmin && (
                  <footer className="mission-card__actions">
                    <Link className="mission-card__action mission-card__action--primary" to={`/misiones/${mision.id}/editar`}>
                      <span>&gt;</span> Editar
                    </Link>
                    <button
                      type="button"
                      className="mission-card__action mission-card__action--ghost"
